...
function getPercentInRange(val: any, min: any, max: any): number {
  const v = Number(val);
  const m = Number(min);
  const x = Number(max);
  if (isNaN(v) || isNaN(m) || isNaN(x)) return 50;
  if (m === x) return 50;
  const range = x - m;
  if (range <= 0) return 50;
  const pct = ((v - m) / range) * 100;
  return isNaN(pct) ? 50 : Math.min(Math.max(pct, 0), 100);
}

function needlePct(m: Biomarker) {
  const val = Number(m.value) || 0;
  const min = Number(m.min) || 1;
  const max = Number(m.max) || (val > 0 ? val * 1.5 : 100);
  const statusClean = (m.status || 'normal').toLowerCase();

  // Fixed zones: low 0-20%, normal 20-80%, high 80-100%
  const LOW_END = 20;
  const HIGH_START = 80;

  if (statusClean === 'low') {
    const ratio = min > 0 ? Math.max(0, Math.min(1, val / min)) : 0.5;
    return LOW_END * (0.1 + ratio * 0.8);
  } else if (statusClean === 'high' || statusClean === 'critical') {
    const excess = max > 0 ? Math.min((val - max) / max, 1) : 0.5;
    return HIGH_START + (100 - HIGH_START) * (0.1 + excess * 0.8);
  } else {
    const safeMax = max > min ? max : min + 1;
    const ratio = Math.max(0, Math.min(1, (val - min) / (safeMax - min)));
    return LOW_END + ratio * (HIGH_START - LOW_END);
  }
}
